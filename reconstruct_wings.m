function [] = reconstruct_wings(init, frames)
...
for n = 1:n_frame
    ...
    flag_eps = 0;
    eps = 500; % initial minimum mask size to extract information
    while 1 % loop twice.  increasing eps to remove a view in the next iteration
        ...
        Pixel_Count = sum(view_keep); % determines in how many images the pixel should be active
...
%         voxel_cord(index_remove,:)=[]; %till here both voxel and volume coordinates are the same
        ...
        SumPixels=pixel_value1*view_keep(1)+pixel_value2*view_keep(2)+pixel_value3*view_keep(3);
        ...
        elseif flag_eps==1 && flag_size1 || flag_size2
            disp('One view already removed and reconstruction could not be completed. Moving to next frame')
            disp('Wing reconstructed as good as possible')
            flag_eps=0;
            break
        else
            flag_eps=0;
