function [Area_ratio]=EstimateWingAreaLossOneView(init,frames,laser_frame)
...
for ii=[sI laser_frame]
    ...
    while 1
        ...
        montage({image3}) % show the bottom view
...
close(f1)
%% Manually trace the wing before damage
f1=figure;
image3 = read(Vreader3, frame_measure(2));
imshow(image3)
BW3 = roipoly;
pixels_intact=sum(BW3,'All');%estimate how many pixels the wing is
close(f1)
%% do the damaged wing
f1=figure;
image3 = read(Vreader3, frame_measure(1));
imshow(image3)
BW3_damaged = roipoly;
BW3_damaged_extra = roipoly; %in case of holes in wing
pixels_damaged=sum(BW3_damaged,'All')-sum(BW3_damaged_extra,'All');%estimate how many pixels the wing is
close(f1)
%% estimate area loss
Area_ratio=pixels_damaged/pixels_intact;
%% save the wing area ratio 
save_location=fullfile(init.folders.root,'WingAreaRatio');
mkdir(save_location)
angle_savepath=fullfile(save_location, 'WingArea.mat');
save(angle_savepath, 'frame_measure','Area_ratio')
end
