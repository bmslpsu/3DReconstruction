...
fly = 2; %change fly number to match the number of fly you are analyzing 
...
[oneWingFlag,error_Flag] = DifferentiateLeftAndRightWingV3(init,7101:7120);
%% UI error detection
clc
ErrorDetectionByUser(init,7101:7120); 
%% find stroke reversal
flag_stroke_reversal=find_stroke_reversal(init,7101,7120,false);
%% UI correction for those pesky large wings (important at stroke reversals)
clc; close all
errorlabel = 6744; 
CorrectForLargeWingsV6(init,7103:7120,zeros(length(flag_stroke_reversal),1),voxel_size); 
...
VerifyReconstruction(init,7101,7102)
%% align the reconstruction with a global reference frame
Rotate3DVoxelsV3(init,7101,7102,true); 
%% convert the hull coordinates to voxel coordinates 
clc; close all
ConvertToVoxelSpacev2(init,7101,7102,true,voxel_size);
%% Analyze the hull
clc; close all
analyze_hull_load_dataV2(init,7101,7102,true,true);
