...
root = 'S:\Public\Wael\Chapter 1\SysID\Intact\Vids';
fly = 4; %change fly number to match the number of fly you are analyzing 
...
[oneWingFlag,error_Flag] = DifferentiateLeftAndRightWingV3(init,7014:eI);
%% UI error detection
clc
ErrorDetectionByUser(init,1421:1450); 
%% 
flag_stroke_reversal=find_stroke_reversal(init,3661,3680,false);
%% UI correction for those pesky large wings (important at stroke reversals)
clc; close all
errorlabel = 6744; 
CorrectForLargeWingsV6(init,1426:1450,zeros(length(flag_stroke_reversal),1),voxel_size); 
...
VerifyReconstruction(init,1421,1450)
%% align the reconstruction with a global reference frame
Rotate3DVoxelsV3(init,1421,1450,true); 
%% convert the hull coordinates to voxel coordinates 
clc; close all
ConvertToVoxelSpacev2(init,1421,1450,true,voxel_size);
%% Analyze the hull
clc; close all
analyze_hull_load_dataV2(init,1421,1450,true,true);
%% Correct chordwise and span vectors (not working)
% clc; close all;
% CorrectChordAutomatic(init,sI,eI)
%% estimates Euler angles
clc
[phi1, theta1, eta1, phi2, theta2, eta2]=EstimateEulerAnglesV2(init,1301,1450);
...
%%
figure(6)
plot(body_angle(20:500))
title("Body Angle of Fly")
xlabel("Frame")
ylabel("Body Angle")