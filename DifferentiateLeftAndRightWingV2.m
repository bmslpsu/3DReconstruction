function [oneWingFlag, error_flag]=DifferentiateLeftAndRightWingV2(init,frames)
...
for ii=frames
    ...
    if length(wing1_cluster)<200 || length(wing2_cluster)<200 || cluster2_len ~=2
    ...
    if length(wing1_cluster)<500 || length(wing2_cluster)<500 %wing size error
        disp('Error flag in this frame set to 1')
        error_flag=1; %sets the flag to 1
    end
    %% save the data
